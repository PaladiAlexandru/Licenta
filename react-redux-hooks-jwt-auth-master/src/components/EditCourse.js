...
import { addCourse, editCourse, getCourse } from '../services/teacher-service';
import { useSelector } from "react-redux";
import Form from 'react-bootstrap/Form';
import { useRef } from "react";
import { getGrades, getWeights } from "../services/grades";
const EditCourse = (props) => {
  ...
  const [numberOfGrades, setNumberOfGrades] = useState(course.nr_of_grades);
  const [weights,setWeights] = useState([])
 
  useEffect(()=>{
    var x= numberOfGrades
 
  },[numberOfGrades])

  useEffect(()=>{
    
    if(course.type == "Final exam only"){
        document.getElementById("gridRadiosFinalExam").checked=true
    }
    
    if(course.type == "Evaluation along the way"){
      getWeights(course.course_id?course.course_id:course.id).then(response =>{
        debugger
        for(var j =0; j< course.nr_of_grades; j++){
          document.getElementsByName("member"+j)[0].value = response.data.rows[j].weight
        }
        setWeights(response.data.rows)
      })
      
      
        document.getElementById("gridRadios1").checked=true
        setChecked1(false)
        document.getElementById("check").defaultValue=course.nr_of_grades
        setNumberOfGrades(course.nr_of_grades)
        displayGrades(course.nr_of_grades)
    }
  ...
  useEffect(()=>{
    ...
    if(checked1)
      ...
      document.getElementById("check").value=course.nr_of_grades
      setNumberOfGrades(course.nr_of_grades)
      displayGrades(course.nr_of_grades)
      for(var j =0; j< course.nr_of_grades; j++){
        debugger
        document.getElementsByName("member"+j)[0].value=weights[j]?.weight
      }
    }
  ...
  const handleSubmit = async(event) => {
    ...
    let data = {
      ...
      nr_of_grades: 0,
      weights: [],
      courseId: course.course_id?course.course_id:course.id
    };
    ...
    if(data.type == "Final exam only"){
      ...
      editCourse(data).then(() => setCourseAdded(true));
  ...
  // You can now access the course data and use it in this component
        ...
        <form ref={formRef} style={{ alignItems: "left", display:"flow"}}>
          <div className="form-group row">
            ...
            <div className="col-sm-10">
              <input type="text" className="form-control" id="courseName" placeholder="Course name" defaultValue={course.name} />
            </div>
          </div>
          <div className="form-group row">
            <label htmlFor="courseDescription" className="col-sm-2 col-form-label">*Description</label>
            <div className="col-sm-10">
              <textarea id="courseDescription" className="form-control" aria-label="A description of the course" defaultValue={course.description}></textarea>
            </div>
          </div>
            ...
            <div className="row">
              ...
              <div className="col-sm-10">
                <div className="form-check">
                  ...
                  <div className="mb-3">
  <label htmlFor="numOfGrades" className="form-label">
    Number of grades
  </label>
  <input
    type="text"
    ...
    id="check"
    placeholder="num of grades"
    value={numberOfGrades || ""}
    onChange={(e) => setNumberOfGrades(e.target.value)}
    onInput={(e)=>displayGrades(e.target.value)}
  />
</div>
  ...
  <div className="alert alert-danger mt-2">
    Grade percentages should add up to 100%
          ...
          <div className="form-group row">
            ...
            <div className="col-sm-10">
  <input type="date" className="form-control" id="examDate" defaultValue={course.exam_date.slice(0, 10)} />
</div>
  ...
  >
    ...
    <div className="card-body">
      ...
      <div className="card-text">
        {courseAdded ? 'Course edited' : `Course couldn't be edited`}
      </div>
    </div>
  </div>
)}
